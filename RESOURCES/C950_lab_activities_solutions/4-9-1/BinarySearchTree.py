...
class Node:
    ...
    # Builds a string representation of the tree rooted at
    # this node. Each node is given by:
    #
    #    (key > LEFT, RIGHT)
    #
    # with LEFT and RIGHT the recursive representation of the
    # two children.
    def __str__(self):
        left = "_" if self.left is None else str(self.left)
        right = "_" if self.right is None else str(self.right)
        if self.left is None and self.right is None:
            return str(self.key)
        else:
            return "(" + str(self.key) + " > " + left + "," + right + ")"
